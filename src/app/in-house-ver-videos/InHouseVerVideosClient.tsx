...
import type { InHouseVerVideoRow } from "@/lib/inHouseVerVideos";
...
export default function InHouseVerVideosClient() {
  const now = Date.now();
  ...
  async function runSearch() {
    ...
    try {
      const res = await fetch("/api/in-house-ver-videos", {
        method: "POST",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-lg font-semibold text-zinc-900">In-house verification videos</h1>
        <p className="mt-1 text-sm text-zinc-600">
          Finds <code className="text-zinc-800">In-house verification passed</code> in VFS Global,
          resolves the activation token per email, then pulls solver{" "}
          <code className="text-zinc-800">[REDIS] Published result</code> video URLs (1–3 attempts,
          success/fail by order).
        </p>
      </div>
      ...
      {data && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Passed (in range)" value={data.totals.passedCount} />
        ...
        <div className="overflow-x-auto rounded-xl border border-zinc-200">
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-zinc-200 bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500">
              <tr>
                <th className="px-3 py-2">Passed at</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {rows.map((row) => (
                <Row key={`${row.email}|${row.passedAt}`} row={row} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      {data && rows.length === 0 && !loading && (
        <p className="text-sm text-zinc-500">No in-house verification passed lines in this window.</p>
      )}
    </div>
  );
}
...
function Row({ row }: { row: InHouseVerVideoRow }) {
  const route =
  ...
  return (
    <tr className="align-top hover:bg-zinc-50/80">
      <td className="whitespace-nowrap px-3 py-2 text-zinc-700">{fmtTime(row.passedAt)}</td>
